...
final class SubstringNextFieldTests: XCTestCase {
...
final class StringReadRecordTests: XCTestCase {
  ...
  func test_recordContainsFieldWithUnbalancedQuotes() {
    let record = "\"one,2,three"
    XCTAssertThrowsError(try record.readRecord()) { error in
      XCTAssertEqual(error as? TransitError, TransitError.quoteExpected)
      XCTAssertEqual(error.localizedDescription,
										 "A quote was expected, but not found")
    }
...
final class StringColorTests: XCTestCase {
  ...
  func test_basic() {
    ...
    XCTAssertEqual(colorString.color, testColor)
  }
...
final class Blah: XCTestCase {
  ...
  func testy3() {
    ...
    XCTAssertThrowsError(try field.assignStringTo(&instance,
																							for: header)) { error in
...
final class StringAndSubstringTests: XCTestCase {
	...
	func test_fieldsFromAgencyWithCTAFeed() {
		...
		""".readHeader() {
			...
			XCTFail("Could not create a test agency")
		}
  }
